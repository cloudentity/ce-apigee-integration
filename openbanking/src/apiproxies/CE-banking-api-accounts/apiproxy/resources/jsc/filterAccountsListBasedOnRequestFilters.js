 ...
 * Return only the accounts that meet the search criteria found in the request and that are included in the consent
 */
...
if (fullListOfAccountsDetails && fullListOfAccountsDetails.length > 0) {
    for (var i = 0; i < fullListOfAccountsDetails.length; i++) {
        if ((consentAccountsList.includes(fullListOfAccountsDetails[i].accountId)) && (accountMeetsSearchCriteria(fullListOfAccountsDetails[i], customerIdFilter, prodCategoryFilter, openStatusFilter, isOwnedFilter))) {
